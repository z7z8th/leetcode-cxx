...
/* 		if (isCached(s, t)) {
		...
		for (auto it=0; it < wl.size(); it++) {
			...
			xformed[it] = true;
			auto mid = it + 1;
			int left = isCached(s, mid) ? lenRef(s, mid) : ladder_len(bw, sw, wl, s, mid);
			int right = isCached(mid, t) ? lenRef(mid, t) : ladder_len(sw, ew, wl, mid, t);
			/* if (s == 0 && t == wl.size() + 1)
				cout << "debug " << left << " + " << right << endl; */
			int len = left + right;
			if (len < min_len) {
				min_len = len;
				min_idx = it;
